import type { ReactNode } from "react";

export type DocProp = {
  name: string;
  type: string;
  default?: string;
  description: string;
  required?: boolean;
};

export const docPropsBySlug: Record<string, DocProp[]> = {
  button: [
    { name: "variant", type: `"primary" | "primary-long" | "secondary-lilac" | "secondary-blue" | "tertiary" | "destructive" | "outline" | "ghost" | "link"`, default: "primary", description: "Visual style. Primary CTAs use lilac." },
    { name: "size", type: `"default" | "sm" | "lg" | "icon"`, default: "default", description: "Height and typography scale." },
    { name: "asChild", type: "boolean", default: "false", description: "Merge props onto a child element (e.g. Next.js Link)." },
    { name: "disabled", type: "boolean", default: "false", description: "Disables interaction and applies muted styling." },
    { name: "className", type: "string", description: "Additional Tailwind classes merged via cn()." },
  ],
  input: [
    { name: "inputSize", type: `"sm" | "default" | "lg"`, default: "default", description: "Field height and font size." },
    { name: "error", type: "string", description: "Error message; applies error border and shows text below." },
    { name: "disabled", type: "boolean", default: "false", description: "Prevents editing." },
    { name: "readOnly", type: "boolean", default: "false", description: "Read-only styling without disabled opacity." },
    { name: "type", type: "string", default: "text", description: "Native input type (text, email, number, etc.)." },
    { name: "placeholder", type: "string", description: "Placeholder copy — pass from i18n." },
    { name: "className", type: "string", description: "Additional classes on the wrapper or input." },
  ],
  checkbox: [
    { name: "checked", type: "boolean", description: "Controlled checked state." },
    { name: "defaultChecked", type: "boolean", description: "Uncontrolled initial checked state." },
    { name: "onCheckedChange", type: "(checked: boolean) => void", description: "Called when the value changes." },
    { name: "disabled", type: "boolean", default: "false", description: "Disables interaction." },
    { name: "id", type: "string", description: "Pair with Label htmlFor for accessibility." },
  ],
  dialog: [
    { name: "open", type: "boolean", description: "Controlled open state." },
    { name: "onOpenChange", type: "(open: boolean) => void", description: "Called when open state changes." },
    { name: "defaultOpen", type: "boolean", description: "Initial open state (uncontrolled)." },
  ],
  tabs: [
    { name: "defaultValue", type: "string", description: "Initially active tab value (uncontrolled)." },
    { name: "value", type: "string", description: "Controlled active tab value." },
    { name: "onValueChange", type: "(value: string) => void", description: "Called when the active tab changes." },
  ],
  toggle: [
    { name: "pressed", type: "boolean", description: "Controlled pressed state for Toggle." },
    { name: "defaultPressed", type: "boolean", description: "Initial pressed state (uncontrolled)." },
    { name: "onPressedChange", type: "(pressed: boolean) => void", description: "Called when Toggle pressed state changes." },
    { name: "variant", type: `"default" | "outline"`, default: "default", description: "Visual style for Toggle and ToggleGroupItem." },
    { name: "size", type: `"default" | "sm" | "lg"`, default: "default", description: "Button size." },
    { name: "type", type: `"single" | "multiple"`, description: "ToggleGroup — single or multi selection." },
    { name: "value", type: "string", description: "ToggleGroup — controlled selected value(s)." },
    { name: "onValueChange", type: "(value: string) => void", description: "ToggleGroup — called when selection changes." },
  ],
  table: [
    { name: "columns", type: "DataTableColumn<T>[]", required: true, description: "Column definitions with header, cell renderer, and optional sortValue." },
    { name: "data", type: "T[]", required: true, description: "Row data array." },
    { name: "getRowKey", type: "(row: T) => string", required: true, description: "Stable unique key per row." },
    { name: "border", type: `"none" | "row" | "striped"`, default: "row", description: "Row border variant." },
    { name: "sortable", type: "boolean", default: "true", description: "Enable column header sorting." },
  ],
  layouts: [
    { name: "children", type: "ReactNode", required: true, description: "Page content rendered inside the layout shell." },
    { name: "headerLinks", type: "NavLink[]", description: "MarketingLayout — top navigation links." },
    { name: "navItems", type: "CabinetNavItem[]", description: "CabinetLayout — sidebar navigation items." },
    { name: "activeHref", type: "string", description: "CabinetLayout — highlights the active nav item." },
  ],
  "app-shell": [
    { name: "logo", type: "ReactNode", description: "Brand mark in the sidebar header." },
    { name: "childProfiles", type: "AppSidebarChildProfile[]", description: "Child profiles shown in the switcher below the logo." },
    { name: "activeChildProfileId", type: "string", description: "Controlled active child profile id." },
    { name: "onChildProfileChange", type: "(profileId: string) => void", description: "Called when the parent selects a different child profile." },
    { name: "childProfileSwitchLabel", type: "string", description: "Accessible label for the child profile switcher trigger." },
    { name: "topItems", type: "AppSidebarLink[]", description: "Primary nav links above collapsible groups." },
    { name: "groups", type: "AppSidebarGroup[]", description: "Collapsible sidebar sections with nested links." },
    { name: "profile", type: "AppSidebarProfile", required: true, description: "Bottom profile menu with avatar and actions." },
    { name: "collapsed", type: "boolean", description: "Controlled sidebar collapsed state." },
    { name: "onCollapsedChange", type: "(collapsed: boolean) => void", description: "Called when sidebar is collapsed or expanded." },
    { name: "LinkComponent", type: "Component", description: "Custom link component (e.g. Next.js Link)." },
    { name: "title", type: "string", description: "AppShellPageHeader — page heading inside the work area." },
    { name: "icon", type: "ReactNode", description: "AppShellPageHeader — icon shown in the colored square before the title." },
    { name: "showDivider", type: "boolean", default: "false", description: "AppShellPageHeader — full-width divider below the header row." },
    { name: "sticky", type: "boolean", default: "false", description: "AppShellPageHeader — optional stick. Default is in-flow on canvas, no white strip." },
    { name: "actions", type: "ReactNode", description: "AppShellPageHeader — trailing actions (filters, buttons)." },
    { name: "notifications", type: "ReactNode", description: "AppShellLayout — notifications panel shown in a popup when the profile bell is clicked." },
    { name: "notificationsOpen", type: "boolean", description: "AppShellLayout — controlled notifications popup open state." },
    { name: "onNotificationsOpenChange", type: "(open: boolean) => void", description: "AppShellLayout — called when the notifications popup opens or closes." },
    { name: "notificationsLabel", type: "string", description: "AppShellLayout — accessible label for the profile bell button." },
    { name: "aiChat", type: "ReactNode", description: "AppShellLayout — AI chat panel rendered in a popup when the AI trigger is clicked." },
    { name: "aiChatOpen", type: "boolean", description: "AppShellLayout — controlled chat open state." },
    { name: "onAiChatOpenChange", type: "(open: boolean) => void", description: "AppShellLayout — called when chat opens or closes." },
    { name: "openAiChatLabel", type: "string", description: "AppShellLayout — accessible label for the AI assistant open button." },
  ],
  "ai-chat": [
    { name: "suggestedPrompts", type: "AiChatSuggestedPrompt[]", description: "Clickable starter prompts shown in the empty state." },
    { name: "onSend", type: "(message: string) => void | Promise<string | AiChatMessage | void>", description: "Called when the user sends a message or picks a prompt. Return assistant text or message object." },
    { name: "quickActions", type: "AiChatQuickAction[]", description: "Pill buttons above the input (Analyze, Find, Create)." },
    { name: "messages", type: "AiChatMessage[]", description: "Controlled message thread." },
    { name: "placeholder", type: "string", description: "Input placeholder text." },
    { name: "disclaimer", type: "string", description: "Footer disclaimer below the input." },
    { name: "onClose", type: "() => void", description: "Called when the close button is pressed. AppShellLayout wires this to close the popup." },
    { name: "sendLabel", type: "string", description: "Accessible label for the send button." },
    { name: "sessionHistoryLabel", type: "string", description: "Accessible label for the sessions menu button." },
    { name: "newSessionLabel", type: "string", description: "Label for starting a new chat session." },
    { name: "noSessionsLabel", type: "string", description: "Empty state when no saved sessions exist." },
    { name: "storage", type: `"local" | "remote"`, default: "local", description: "Persist sessions in browser localStorage or on your backend via remoteStorage." },
    { name: "remoteStorage", type: "AiChatRemoteStorage", description: "Required when storage=\"remote\". load/save hooks wired to your API." },
    { name: "sessionStorageKey", type: "string | false", default: "safelagoon-ai-chat", description: "localStorage key when storage=\"local\". Pass false to disable persistence." },
    { name: "sessionRetentionDays", type: "number", default: "30", description: "Auto-expire saved sessions after this many days." },
    { name: "onSessionChange", type: "(sessionId: string) => void", description: "Called when the active session changes." },
  ],
  header: [
    { name: "logo", type: "ReactNode", description: "Logo or brand element." },
    { name: "links", type: "{ label: string; href: string }[]", description: "Desktop navigation links." },
    { name: "actions", type: "ReactNode", description: "Right-side actions (CTA buttons)." },
    { name: "menuItems", type: "BurgerMenuItem[]", description: "Mobile fullscreen menu items." },
    { name: "sticky", type: "boolean", default: "true", description: "Stick header to top on scroll." },
  ],
  "burger-menu": [
    { name: "items", type: "BurgerMenuItem[]", required: true, description: "Navigation items shown in the overlay." },
    { name: "triggerLabel", type: "string", description: "Accessible label for the menu button." },
    { name: "closeLabel", type: "string", description: "Accessible label for the close button." },
  ],
  sidebar: [
    { name: "items", type: "SidebarItem[]", required: true, description: "Navigation items with optional icons." },
    { name: "header", type: "ReactNode", description: "Slot above nav items." },
    { name: "footer", type: "ReactNode", description: "Slot below nav items." },
    { name: "className", type: "string", description: "Additional classes on the aside element." },
  ],
  drawer: [
    { name: "open", type: "boolean", description: "Controlled open state." },
    { name: "onOpenChange", type: "(open: boolean) => void", description: "Called when open state changes." },
    { name: "side", type: `"top" | "right" | "bottom" | "left"`, default: "right", description: "Edge the panel slides from." },
  ],
  title: [
    { name: "title", type: "string", required: true, description: "Page heading text — pass from i18n." },
    { name: "icon", type: "ReactNode", description: "Optional icon in the brand-blue square beside the title." },
    { name: "subtitle", type: "ReactNode", description: "Optional supporting copy below the icon + title row." },
    { name: "showDivider", type: "boolean", default: "false", description: "Show a full-width border below the title block." },
    { name: "className", type: "string", description: "Additional classes on the root wrapper." },
  ],
  "date-picker": [
    { name: "value", type: "Date | undefined", description: "Selected date (controlled)." },
    { name: "onChange", type: "(date: Date | undefined) => void", description: "Called when the user picks a date." },
    { name: "label", type: "string", description: "Field label — pass from i18n." },
    { name: "placeholder", type: "string", description: "Placeholder when no date is selected." },
    { name: "disabled", type: "boolean", default: "false", description: "Disables the trigger." },
    { name: "error", type: "string", description: "Validation error message." },
  ],
  map: [
    { name: "apiKey", type: "string", required: true, description: "Google Maps JavaScript API key with Maps API enabled." },
    { name: "center", type: "{ lat: number; lng: number }", required: true, description: "Map center coordinates." },
    { name: "radius", type: "number", required: true, description: "GeofenceMap — circle radius in meters." },
    { name: "onRadiusChange", type: "(radius: number) => void", description: "GeofenceMap — called when the user resizes the circle." },
    { name: "onCenterChange", type: "(center: LatLngLiteral) => void", description: "GeofenceMap — called when the circle center moves." },
    { name: "editable", type: "boolean", default: "false", description: "GeofenceMap — allow dragging/resizing the geofence." },
    { name: "height", type: "number | string", description: "Map container height in pixels or CSS value." },
    { name: "showPin", type: "boolean", default: "true", description: "MiniMap — show LocationPin at center." },
  ],
  "log-card": [
    { name: "icon", type: "ReactNode", required: true, description: "Leading icon (brand icon recommended)." },
    { name: "title", type: "string", required: true, description: "Event title." },
    { name: "description", type: "string", required: true, description: "Event detail or URL." },
    { name: "timestamp", type: "string", required: true, description: "Formatted time string from app i18n." },
    { name: "blocked", type: "boolean", default: "false", description: "Shows blocked badge styling." },
    { name: "blockedLabel", type: "string", default: "Blocked", description: "Badge text when blocked." },
    { name: "screenshot", type: "string", description: "Optional screenshot thumbnail URL." },
  ],
  "profile-card": [
    { name: "name", type: "string", required: true, description: "Child profile display name." },
    { name: "os", type: `"android" | "ios"`, required: true, description: "Device OS for badge icon." },
    { name: "batteryPercent", type: "number", description: "Battery level 0–100." },
    { name: "pin", type: "string", description: "Device PIN code display." },
    { name: "avatarSrc", type: "string", description: "Profile photo URL or data URL." },
    { name: "avatarFallback", type: "string", required: true, description: "Initials shown when no photo is set." },
    { name: "onAvatarUpload", type: "(file: File) => void", description: "Called when a file is picked. Enables avatar upload UI on the photo." },
    { name: "onAvatarCropComplete", type: "(dataUrl: string) => void", description: "Called with the cropped JPEG data URL after the user saves." },
    { name: "avatarUploadLabel", type: "string", description: "Accessible label for the avatar upload button." },
    { name: "avatarCropTitle", type: "string", description: "Crop dialog title." },
    { name: "avatarCropLabel", type: "string", description: "Crop dialog save button label." },
    { name: "avatarCropCancelLabel", type: "string", description: "Crop dialog cancel button label." },
    { name: "avatarCropZoomLabel", type: "string", description: "Crop dialog zoom slider label." },
    { name: "selected", type: "boolean", default: "false", description: "Highlighted selection state." },
  ],
  "rule-card": [
    { name: "title", type: "string", required: true, description: "Rule name." },
    { name: "mode", type: `"view" | "edit"`, default: "view", description: "Read-only or editable layout." },
    { name: "enabled", type: "boolean", description: "Whether the rule is active." },
    { name: "onEnabledChange", type: "(enabled: boolean) => void", description: "Toggle handler in edit mode." },
    { name: "scheduleLabel", type: "string", description: "Schedule section heading." },
    { name: "children", type: "ReactNode", description: "Custom rule body (schedule grid, limits, etc.)." },
  ],
  "schedule-grid": [
    { name: "value", type: "boolean[][]", required: true, description: "7×24 grid of allowed/blocked cells." },
    { name: "onChange", type: "(value: boolean[][]) => void", description: "Called when cells are toggled." },
    { name: "readOnly", type: "boolean", default: "false", description: "Disable cell interaction." },
    { name: "weekdayLabels", type: "string[]", description: "Localized day abbreviations (Mon–Sun)." },
  ],
  "sign-in-form": [
    { name: "emailLabel", type: "string", required: true, description: "Email field label." },
    { name: "passwordLabel", type: "string", required: true, description: "Password field label." },
    { name: "submitLabel", type: "string", required: true, description: "Submit button text." },
    { name: "forgotPasswordLabel", type: "string", description: "Forgot password link text." },
    { name: "onSubmit", type: "(data: { email: string; password: string }) => void", description: "Form submit handler." },
    { name: "emailError", type: "string", description: "Email validation message." },
    { name: "passwordError", type: "string", description: "Password validation message." },
    { name: "isLoading", type: "boolean", default: "false", description: "Shows loading state on submit." },
  ],
  gallery: [
    { name: "items", type: "MediaGalleryItem[]", required: true, description: "Photos with src, alt, optional flagged/AI Shield metadata." },
    { name: "selectable", type: "boolean", default: "false", description: "Enable multi-select checkboxes." },
    { name: "selectedIds", type: "string[]", description: "Controlled selection IDs." },
    { name: "onSelectionChange", type: "(ids: string[]) => void", description: "Called when selection changes." },
    { name: "emptyTitle", type: "string", description: "Empty state heading." },
    { name: "emptyDescription", type: "string", description: "Empty state body copy." },
  ],
  "screen-mirror": [
    { name: "frames", type: "ScreenMirrorFrame[]", required: true, description: "Captured frames with id, src, timestamp." },
    { name: "activeFrameId", type: "string", description: "Currently displayed frame ID." },
    { name: "onActiveFrameChange", type: "(id: string) => void", description: "Called when user navigates frames." },
    { name: "sessionLabel", type: "string", description: "Session title above the viewer." },
    { name: "timelineLabel", type: "string", description: "Filmstrip section label." },
  ],
  "event-timeline": [
    { name: "events", type: "EventTimelineEntry[]", required: true, description: "Mixed event feed items." },
    { name: "types", type: "EventTimelineType[]", required: true, description: "Filter chip definitions." },
    { name: "filterLabel", type: "string", description: "Filter section label." },
    { name: "renderEvent", type: "(event: EventTimelineEntry) => ReactNode", required: true, description: "Render function — typically returns LogCard." },
    { name: "groupByDate", type: "boolean", default: "true", description: "Group events under date headings." },
  ],
  notifications: [
    { name: "title", type: "string", default: "Notifications", description: "Panel heading." },
    { name: "groups", type: "NotificationGroup[]", description: "Grouped notification lists with date labels." },
    { name: "settingsLabel", type: "string", description: "Accessible label for the settings button." },
    { name: "onSettingsClick", type: "() => void", description: "Called when settings is clicked." },
    { name: "onClose", type: "() => void", description: "Called when close is clicked. AppShellLayout wires this automatically." },
    { name: "onNotificationClick", type: "(notification: AppNotification) => void", description: "Called when a notification row is clicked." },
    { name: "emptyLabel", type: "string", description: "Copy shown when there are no notifications." },
  ],
  toast: [
    { name: "position", type: `"top-left" | "top-center" | "top-right" | "bottom-left" | "bottom-center" | "bottom-right"`, default: "bottom-right", description: "Toaster — viewport corner where toasts stack." },
    { name: "duration", type: "number", default: "4000", description: "Default auto-dismiss time in milliseconds." },
    { name: "closeButton", type: "boolean", default: "true", description: "Toaster — show a dismiss button on each toast." },
    { name: "expand", type: "boolean", default: "false", description: "Toaster — expand the toast stack on hover." },
    { name: "visibleToasts", type: "number", default: "3", description: "Maximum number of visible toasts at once." },
    { name: "toast()", type: "(message, options?) => id", description: "Show a default toast. Pass title copy as message; description and action via options." },
    { name: "toast.success()", type: "(message, options?) => id", description: "Success variant with green accent." },
    { name: "toast.error()", type: "(message, options?) => id", description: "Error variant with destructive accent." },
    { name: "toast.info()", type: "(message, options?) => id", description: "Info variant with brand-blue accent." },
    { name: "toast.warning()", type: "(message, options?) => id", description: "Warning variant with yellow accent." },
    { name: "toast.dismiss()", type: "(id?) => void", description: "Dismiss one toast by id, or all when id is omitted." },
  ],
  portal: [
    { name: "screen", type: `proto.js path, e.g. "/home" | "/feed/aishield" | "/settings/billing/trial"`, description: "Constructor screen. Every proto.js route. Hubs and crumbs call onGo." },
    { name: "PlaceEditor.type", type: `"HOME" | "SCHOOL" | "UNKNOWN" | "DANGER" | "YARD"`, description: "Proto chips Home / School / Place / Danger / Yard. Live GEO_PLACE_TYPES map when wiring SPA." },
    { name: "PlaceEditor.radius", type: "number", description: "50–3000 m, step 10. Same circle the child device uses." },
    { name: "ChildProfileForm", type: "name + emoji avatar + Boy/Girl + age band + Android/iOS", description: "Family editor. Pairing is a follow-on, not a form field." },
    { name: "DotCrumbs", type: "string | { label, href }[]", description: "13px crumbs with · separators. Last item is aria-current=page. AppShellPageHeader still renders the 28/22px h1 below." },
    { name: "PlanCard + SectionTitle", type: "Plans / Add-ons / Payment method sit above the cards", description: "Billing contract from the accepted proto." },
  ],
  "portal-atoms": [
    { name: "BrandMark.size", type: `"sidebar" | "auth"`, default: `"sidebar"`, description: "28px sidebar mark vs 36px auth logo. hideLabel for the 72px rail." },
    { name: "AppNavIcon.kind", type: `"home" | "feed" | "rules" | "settings"`, required: true, description: "Proto stroke icons. Pass as AppShellLayout topItems[].icon." },
    { name: "PanelGrid", type: "children", description: "3 / 2 / 1 columns from AppShell data-shell-mode. Wrap SectionCard KPIs." },
    { name: "AppMark.kind", type: `"tiktok" | "roblox" | "minecraft" | "youtube"`, description: "40×40 proto .sr-icon fills." },
    { name: "VideoThumb", type: "src + duration", description: "96×54 YouTube row thumb with play overlay." },
    { name: "PortalSelect", type: "native <select>", description: "40px proto field. Blue 3px focus ring. Not Radix Select." },
    { name: "ToggleRow", type: "title + subtitle + checked", description: "Settings row + 44×24 lilac switch. Controlled or defaultChecked." },
    { name: "PayCard", type: "label + selected", description: "Billing radio. Wrap in PayCardGroup." },
    { name: "MapToolbar", type: `"draw" | "pan"`, description: "Draw zone / Move map chips + hint." },
    { name: "TextAction", type: "onClick | href", description: "+ Add place / + Add child under a StrokeGroup." },
    { name: "AuthCard", type: "AuthPage + BrandMark + AuthTitle", description: "420px / 20px radius proto sign-in card." },
    { name: "FeedBubble", type: `"in" | "out"`, description: "Activity chat. Pair with ChatDayLabel." },
  ],
  "device-mode": [
    { name: "value", type: `"allow" | "rules" | "block"`, required: true, description: "Selected mode. Maps to ProfileControlRequest.state 2 / 0 / 1." },
    { name: "onValueChange", type: "(value: DeviceMode) => void", description: "Called when the parent picks another exclusive mode." },
    { name: "disabled", type: "boolean", default: "false", description: "Unlinked device — control stays visible but locked." },
    { name: "busy", type: "boolean", default: "false", description: "Pending write — same as disabled." },
    { name: "captions", type: "Partial<Record<DeviceMode, string>>", description: "Override the three status captions under the bar." },
    { name: "caption", type: "ReactNode", description: "Force a caption (e.g. extra-time timer on allow)." },
  ],
  "feed-stacks": [
    { name: "kind", type: `"apps" | "screentime" | "gallery" | "internet" | "youtube" | "places" | "aishield" | "chats"`, description: "ActivityStack — visual pile. Fallback is colored layers." },
    { name: "previews", type: "ActivityStackPreview[]", description: "Icons / thumbs for the stack." },
    { name: "alert", type: "boolean", default: "false", description: "Loud AI Shield / Needs review treatment." },
    { name: "expanded", type: "boolean", description: "ActivityDaySection — Today and Yesterday start open; older days stay collapsed." },
    { name: "items", type: "FilterChip[]", required: true, description: "FilterChips — All + kinds. Keep Critical as an overlay chip." },
  ],
  "kpi-card": [
    { name: "icon", type: "ReactNode", required: true, description: "Metric icon shown in the tinted square." },
    { name: "value", type: "string | number", required: true, description: "Primary metric value." },
    { name: "label", type: "string", required: true, description: "Metric label — pass from i18n." },
    { name: "trendLabel", type: "string", description: "Optional trend line, e.g. '↓ 18% vs yesterday'." },
    { name: "trendDirection", type: `"up" | "down" | "neutral"`, default: "neutral", description: "Trend color: green, red, or muted." },
    { name: "onClick", type: "() => void", description: "Makes the card clickable with hover/focus styles." },
  ],
};

export function getDocProps(slug: string): DocProp[] {
  return docPropsBySlug[slug] ?? [];
}
